...
class CameraConnectionThread(threading.Thread):
    ...
    def _update_db_status(self, status: str):
        """
        Pushes live connection status directly to the PostgreSQL database.
        """
        db = SessionLocal()
        try:
            ...
            if camera and camera.status != status:
                ...
                db.commit()
                logger.info(f"[{self.name}] Database status synced to: {status}")
        except Exception as e:
            logger.error(f"[{self.name}] Failed to sync status to database: {e}")
        finally:
            db.close()
