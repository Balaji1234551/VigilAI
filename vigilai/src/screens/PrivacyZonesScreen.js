      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.infoCard}>
          <Text style={styles.infoText}>
            Draw blackout boxes on specific areas of your camera feed to permanently hide sensitive zones. These areas will always appear as black rectangles in recordings and live feeds.
          </Text>
          <View style={styles.usageRow}>
            <EyeOff size={14} color="#94A3B8" />
            <Text style={styles.usageText}>Common uses: TVs showing sensitive info, neighbor's windows, private spaces</Text>
          </View>
        </View>
        ...
        <View style={styles.listCard}>
          <View style={styles.listHeader}>
            <View style={styles.row}>
              ...
              <Text style={styles.listTitle}>Active Privacy Zones ({zones.length})</Text>
            </View>
          ...
          {zones.map((zone, index) => (
            <ZoneItem 
              ...
              onDelete={() => deleteZone(zone.id, zone.title)}
              isLast={index === zones.length - 1}
            />
          ))}

          {zones.length === 0 && (
            <Text style={{ color: '#64748B', textAlign: 'center', marginVertical: 20 }}>No privacy zones configured.</Text>
          )}
        </View>

        {/* How to Create Section */}
        <View style={styles.howCard}>
          <Text style={styles.howTitle}>How to Create a Privacy Zone</Text>
          <StepItem number="1" text="Select the camera you want to configure" />
          <StepItem number="2" text="View the live feed and tap “Draw Zone”" />
          <StepItem number="3" text="Drag to create a rectangle over the area you want to hide" />
          <StepItem number="4" text="Name the zone and save - the area will be permanently blacked out" />
        </View>

        {/* Important Notes Card */}
        <View style={styles.notesCard}>
          <Text style={styles.notesTitle}>Important Notes</Text>
          <NoteItem text={<Text>Privacy zones are applied <Text style={styles.boldNote}>before</Text> AI detection - the AI never sees these areas</Text>} />
          <NoteItem text="Once blacked out, these areas cannot be un-blurred in past recordings" />
          <NoteItem text="You can move, resize, or delete zones at any time" />
        </View>
      </ScrollView>
      ...
      >
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <Text style={styles.modalTitle}>Map New Privacy Zone</Text>
            <Text style={styles.modalSubtitle}>Normally, you would draw the rectangle on the live feed here. For now, enter the zone details.</Text>
            
            <TextInput
              style={styles.modalInput}
              placeholder="Zone Name (e.g. Neighbor's Yard)"
            ...
            <TextInput
              style={styles.modalInput}
              placeholder="Location/Camera (e.g. Backyard Cam)"
              placeholderTextColor="#64748B"
              value={newLoc}
            ...
            <View style={styles.modalButtonRow}>
              ...
              <TouchableOpacity style={styles.modalSaveBtn} onPress={handleAddZone}>
...
const styles = StyleSheet.create({
  ...
  previewContainer: { height: 110, backgroundColor: '#0F172A', borderRadius: 15, marginTop: 15, justifyContent: 'center', alignItems: 'center', position: 'relative', overflow: 'hidden' },
