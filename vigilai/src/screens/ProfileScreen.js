    ...
    const unsubscribe = navigation.addListener('focus', () => {
      const fetchUser = async () => {
        try {
          ...
          // Dynamically fetch Active Cameras
          const storedCameras = await AsyncStorage.getItem('cameras');
          if (storedCameras) {
            const camerasArr = JSON.parse(storedCameras);
            // Count cameras that are not offline
            const active = camerasArr.filter(c => c.status !== 'offline' && c.status !== 'Offline').length;
            setActiveCamerasCount(active > 0 ? active : camerasArr.length);
          } else {
            setActiveCamerasCount(0); // Real app behavior: 0 if no cameras
          }
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.statsRow}>
          <ProfileStat icon={<Camera size={20} color="#00E5FF" />} val={activeCamerasCount.toString()} label={t('activeCameras')} />
          <ProfileStat icon={<Calendar size={20} color="#00C853" />} val={daysActiveCount.toString()} label={t('daysActive')} />
          <ProfileStat icon={<ShieldAlert size={20} color="#FFD600" />} val={preventedCount.toString()} label={t('prevented')} />
        </View>
        ...
        <View style={styles.menuCard}>
          ...
          <MenuItem icon={<CreditCard size={20} color="#94A3B8" />} label={t('subscription')} rightText="Free" onPress={() => navigation.navigate('Subscription')} />
          <MenuItem icon={<HelpCircle size={20} color="#94A3B8" />} label={t('helpSupport')} onPress={() => navigation.navigate('HelpSupport')} />
          <MenuItem icon={<FileText size={20} color="#94A3B8" />} label={t('termsPrivacy')} onPress={() => navigation.navigate('TermsPrivacy')} isLast />
        </View>
      ...
      <View style={styles.tabBar}>
        ...
        <TabItem icon={<Bell size={24} color="#94A3B8" />} label={t('alerts')} onPress={() => navigation.navigate('Alerts')} />
        <TabItem icon={<BarChart2 size={24} color="#94A3B8" />} label={t('analytics')} onPress={() => navigation.navigate('Analytics')} />
        <TabItem icon={<User size={24} color="#00E5FF" />} label={t('profile')} active />
      </View>
    </SafeAreaView>
  );
}
