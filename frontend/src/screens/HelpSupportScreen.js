    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView style={{ flex: 1 }} contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.channelRow}>
          <SupportChannel icon={<MessageCircle size={28} color="#00E5FF" />} label="Live Chat" onPress={() => handleSupportLink('chat')} />
          <SupportChannel icon={<BookOpen size={28} color="#00E5FF" />} label="Documentation" onPress={() => handleSupportLink('docs')} />
          <SupportChannel icon={<Video size={28} color="#00E5FF" />} label="Tutorials" onPress={() => handleSupportLink('tutorials')} />
        </View>
        ...
        {faqData.map((section, idx) => (
          <View key={idx} style={styles.faqSection}>
            ...
            {section.items.filter(item => item.question.toLowerCase().includes(searchQuery.toLowerCase())).map((item, i) => (
              <FAQItem 
                key={item.id}
        ...
        <View style={styles.contactCard}>
          ...
          <Text style={styles.contactSubtitle}>
            Our dedicated security team is available 24/7 to assist you with any technical or account issues.
          </Text>
        ...
        <KeyboardAvoidingView style={styles.chatContainer} behavior={Platform.OS === "ios" ? "padding" : "height"}>
          ...
          <View style={styles.chatInputContainer}>
            ...
            <TouchableOpacity style={styles.sendBtn} onPress={handleSendMessage}>
              <Send size={20} color="#0A0E17" style={{ marginLeft: -2 }} />
            </TouchableOpacity>
          </View>
...
const styles = StyleSheet.create({
  ...
  faqSection: { backgroundColor: '#161B29', borderRadius: 15, marginBottom: 20, borderWidth: 1, borderColor: '#1E293B' },
