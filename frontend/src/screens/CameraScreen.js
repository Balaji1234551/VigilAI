...
export default function CameraScreen({ navigation }) {
  const { cameras, removeCamera } = useGlobalContext();
  ...
  const mappedCameras = React.useMemo(() => {
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.listContainer}>
          {mappedCameras.length === 0 ? (
            ...
            mappedCameras.filter(c => c.title.toLowerCase().includes(searchQuery.toLowerCase()) || c.subtitle.toLowerCase().includes(searchQuery.toLowerCase())).map(cam => (
              <CameraCard 
                key={cam.id}
                title={cam.title} 
                ...
                onMenuPress={() => {
                  if (Platform.OS === 'web') {
                    if (window.confirm(`Are you sure you want to delete ${cam.title}?`)) {
                      removeCamera(cam.id).catch(e => window.alert('Failed to delete camera.'));
                    }
                  } else {
